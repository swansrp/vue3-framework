      ...
      <a-select
        ...
        <template #dropdownRender="{ menuNode: menu }" v-if="isMultiple">
          ...
          <div class="staff-select-operation-btn-list" @mousedown="e => e.preventDefault()">
            <a-button size="small" class="confirm-btn" @click="closeSelectBox">确定</a-button>
...
const {layoutMode, width} = toRefs(props)
...
const isMultiple = computed(() => props.isMultiple || props.isMultiple === undefined)
...
const handleStaffChange = (option: ValueLabel | null) => {
  if (isMultiple.value) emit('update:staffListValue', staffListValue.value)
    ...
    else staffListValue.value = [{label: option[option.length-1].label, value: option[option.length-1].value}]
