...
import type {
  ChartDataItem,
  DataMetric,
} from "@/framework/components/common/Portal/dashboard/type/ChartTypes";
...
export default defineComponent({
  ...
  props: {
    ...
    dimensionValueMap: {
      type: Object as () =>
        | { first?: Record<string, string>; second?: Record<string, string> }
        | undefined,
      default: undefined,
    },
    // 是否为指标饼图模式（扇区=数据指标字段）
  ...
  setup(props, { emit }) {
    ...
    const processChartData = (data: ChartDataItem[]) => {
      ...
      // 提取维度分组，保持原始顺序
      const firstDimensionGroups = [
        ...new Set(flattenedData.map((item) => item.firstDimension)),
      ];
    ...
    // 构建“全量数据” tooltip HTML：列出所有扇区的数值与占比，并高亮当前选中项
    const buildFullTooltipHtml = (
      pieData: any[],
      currentName: string,
      headerText: string,
      getUnitForItem: (item: any, index: number) => string,
      formatValue: (value: number, item: any, index: number) => string,
      totalText: string
    ): string => {
      ...
      pieData.forEach((d, idx) => {
        ...
        const percent =
          totalValue > 0 ? ((d.value / totalValue) * 100).toFixed(2) : "0.00";
        const color = d.itemStyle?.color || "#1890ff";
        const unit = getUnitForItem(d, idx);
        ...
        const currentTag = isCurrent
          ...
          : "";
        html += `<div style="${rowStyle}">${marker}<span style="vertical-align:middle;">${d.name}: ${formatValue(d.value, d, idx)}${unit} (${percent}%)</span>${currentTag}</div>`;
      });
    ...
    const generatePieChartOption = (processedData: any): echarts.EChartsOption => {
      ...
      if (props.isMetricsMode) {
        ...
        const pieData = metrics.map((metric, idx) => {
          ...
          const item = flattenedData.find(
            (d: any) => d.statisticType === metric.dataName
          );
          const originalValue = item ? item.statistic : 0;
          const { unit: unitDivisor } = metric.unitConfig
            ? parseUnitConfig(metric.unitConfig)
            : { unit: 1 };
          const convertedValue = metric.unitConfig
            ? originalValue / unitDivisor
          ...
          tooltip: {
            ...
            formatter: (params: any) => {
              ...
              const getMetricByName = (name: string) =>
                metrics.find((m) => m.dataName === name) || metrics[0];
              const fmtWith = (v: number, metric: any) => {
              ...
              const currentMetric = getMetricByName(params.name);
              const totalUnit = getEffectiveUnit(currentMetric);
              return buildFullTooltipHtml(
                pieData,
                params.name,
                props.title || "指标占比",
                (d) => getEffectiveUnit(getMetricByName(d.name)) || "",
                (v, d) => fmtWith(v, getMetricByName(d.name)),
                `总计：${fmtWith(totalValue, currentMetric)}${totalUnit || ""}`
              );
            },
          },
          legend: {
      ...
      const pieData = categories
        .map((category: string, categoryIndex: number) => {
          ...
          const item = flattenedData.find(
            (d: any) =>
              d.firstDimension === category && d.statisticType === pieMetric.dataName
          );
          ...
          // 对数据值进行单位转换
          const originalValue = item ? item.statistic : 0;
          const { unit: unitDivisor } = pieMetric.unitConfig
            ? parseUnitConfig(pieMetric.unitConfig)
            : { unit: 1 };
          // 对于非金额指标，确保结果为整数；对于金额指标，保持精度
          const convertedValue = pieMetric.unitConfig
            ? originalValue / unitDivisor
            : Math.round(originalValue);
      ...
      // 计算总额（基于过滤/排序后的 pieData）
      const totalValue = pieData.reduce(
        (sum: number, d: any) => sum + (typeof d.value === "number" ? d.value : 0),
        0
      );
      ...
      return {
        ...
        tooltip: {
          ...
          formatter: (params: any) => {
            ...
            const totalValue = (Array.isArray(pieData) ? pieData : []).reduce(
              (sum, d: any) => sum + (typeof d?.value === "number" ? d.value : 0),
              0
            );
            return buildFullTooltipHtml(
              pieData,
