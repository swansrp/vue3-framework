        ...
        <div class="header-row">
          ...
          <div class="indicator-name-input">
            <span class="input-label"
              ><span class="required-star">* </span>指标名称：</span
            >
            <a-input
            ...
            />
            <span v-if="indicatorNameError" class="error-message">{{
              indicatorNameError
            }}</span>
          </div>
  ...
  async (newVal) => {
    ...
    if (newVal) {
      ...
      setTimeout(() => {
        if (
          dashboardRef.value &&
          typeof dashboardRef.value.forceRecalculateLayout === "function"
        ) {
  ...
  try {
    ...
    if (!savedConfig) {
      ...
      if (currentNode && currentNode.indicator) {
        try {
          savedConfig =
            typeof currentNode.indicator === "string"
...
const handleSaveConfig = async () => {
  try {
    ...
    const indicatorData: Partial<IndicatorNode> = {
      ...
      indicator: skipChartGeneration.value
        ? ""
        : JSON.stringify(dimensionIndicatorsFilter),
    };
