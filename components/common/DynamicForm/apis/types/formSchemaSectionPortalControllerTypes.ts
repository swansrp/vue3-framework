// ==================== Form Schema Section Portal Controller Types ====================
// 项目: mpbe-api
// 文件: formSchemaSectionPortalControllerTypes.ts
// Tag: 表单配置 - 表单区块
...
/**
 * 分页响应类型 - 原始类型: Page«FormSchemaSectionVO»
 * @description 这个接口描述了分页响应的结构，其中 records 字段包含 FormSchemaSectionVO 类型的数组
 */
export interface PageFormSchemaSectionVO {
  countId?: string
  current?: number
  maxLimit?: number
  ...
  orders?: OrderItem[]
  pages?: number
  /** 分页数据列表 - 包含 FormSchemaSectionVO 类型的数组 */
  records?: FormSchemaSectionVO[]
  searchCount?: boolean
  size?: number
  total?: number
}
...
export interface FormSchemaSectionVO {
  /** 创建时间 */
  createAt?: string
  /** 创建人工号 */
  createBy?: string
...
export type PageFormSchemaSectionVOResponse = ResponseDataType & {
  payload: PageFormSchemaSectionVO
}

export type PageFormSchemaSectionVOListResponse = ResponseDataType & {
  payload: PageFormSchemaSectionVO[]
}

export type PageFormSchemaSectionVOPageResponse = ResponseDataType & {
  payload: PageFormSchemaSectionVO
}

export type FormSchemaSectionVOResponse = ResponseDataType & {
  payload: FormSchemaSectionVO
}

export type FormSchemaSectionVOListResponse = ResponseDataType & {
  payload: FormSchemaSectionVO[]
}

export type FormSchemaSectionVOPageResponse = ResponseDataType & {
  payload: {
    records: FormSchemaSectionVO[]
    total: number
    currentPage: number
