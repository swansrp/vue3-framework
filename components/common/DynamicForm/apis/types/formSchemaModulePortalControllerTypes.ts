// ==================== Form Schema Module Portal Controller Types ====================
// 项目: mpbe-api
// 文件: formSchemaModulePortalControllerTypes.ts
// Tag: 表单配置 - 表单模块
...
/**
 * 分页响应类型 - 原始类型: Page«FormSchemaModuleVO»
 * @description 这个接口描述了分页响应的结构，其中 records 字段包含 FormSchemaModuleVO 类型的数组
 */
export interface PageFormSchemaModuleVO {
  countId?: string
  current?: number
  maxLimit?: number
  ...
  orders?: OrderItem[]
  pages?: number
  /** 分页数据列表 - 包含 FormSchemaModuleVO 类型的数组 */
  records?: FormSchemaModuleVO[]
  searchCount?: boolean
  size?: number
  total?: number
}
...
export interface FormSchemaModuleVO {
  /** 创建时间 */
  createAt?: string
  /** 创建人工号 */
  createBy?: string
...
export type PageFormSchemaModuleVOResponse = ResponseDataType & {
  payload: PageFormSchemaModuleVO
}

export type PageFormSchemaModuleVOListResponse = ResponseDataType & {
  payload: PageFormSchemaModuleVO[]
}

export type PageFormSchemaModuleVOPageResponse = ResponseDataType & {
  payload: PageFormSchemaModuleVO
}

export type FormSchemaModuleVOResponse = ResponseDataType & {
  payload: FormSchemaModuleVO
}

export type FormSchemaModuleVOListResponse = ResponseDataType & {
  payload: FormSchemaModuleVO[]
}

export type FormSchemaModuleVOPageResponse = ResponseDataType & {
  payload: {
    records: FormSchemaModuleVO[]
    total: number
    currentPage: number
