// ==================== Form Data Section Instance Portal Controller Types ====================
// 项目: mpbe-api
// 文件: formDataSectionInstancePortalControllerTypes.ts
// Tag: 表单数据 - 区块实例
...
/**
 * 分页响应类型 - 原始类型: Page«FormDataSectionInstanceVO»
 * @description 这个接口描述了分页响应的结构，其中 records 字段包含 FormDataSectionInstanceVO 类型的数组
 */
export interface PageFormDataSectionInstanceVO {
  countId?: string
  current?: number
  maxLimit?: number
  ...
  orders?: OrderItem[]
  pages?: number
  /** 分页数据列表 - 包含 FormDataSectionInstanceVO 类型的数组 */
  records?: FormDataSectionInstanceVO[]
  searchCount?: boolean
  size?: number
  total?: number
}
...
export interface FormDataSectionInstanceVO {
  /** 审核时间 */
  confirmAt?: string
  /** 审核人 */
  confirmBy?: string
...
export type PageFormDataSectionInstanceVOResponse = ResponseDataType & {
  payload: PageFormDataSectionInstanceVO
}

export type PageFormDataSectionInstanceVOListResponse = ResponseDataType & {
  payload: PageFormDataSectionInstanceVO[]
}

export type PageFormDataSectionInstanceVOPageResponse = ResponseDataType & {
  payload: PageFormDataSectionInstanceVO
}

export type FormDataSectionInstanceVOResponse = ResponseDataType & {
  payload: FormDataSectionInstanceVO
}

export type FormDataSectionInstanceVOListResponse = ResponseDataType & {
  payload: FormDataSectionInstanceVO[]
}

export type FormDataSectionInstanceVOPageResponse = ResponseDataType & {
  payload: {
    records: FormDataSectionInstanceVO[]
    total: number
    currentPage: number
