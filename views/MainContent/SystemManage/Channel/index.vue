<template>
  <left-list-right-table
    @get-left-item-list="getLeftItemList" @get-table-by-item="getTableByItem" @update-current-item-value="updateCurrentItemValue"
    :columns="surelyTableColumns" :update-table-flag="updateTableFlag">
    <template #top_btn_list>
      <a-button type="primary" @click="addTableItem" style="float: right; margin-bottom: 10px;">新增</a-button>
    </template>
    <template #table_btn_list="{record}">
      <a-button size="small" type="primary" @click="handleEditTable(record)">编辑</a-button>
      <delete-pop-confirm @delete-event="handleDeleteTable(record)" />
    </template>
  </left-list-right-table>
  <dialog-box v-model:visible="addTableItemFinishedVisible" title="信息" :width="800">
    <a-descriptions layout="vertical" bordered :column="1" :label-style="{fontWeight: 'bold'}">
      <a-descriptions-item label="应用标识">{{ newTableItemInfo.appKey }}</a-descriptions-item>
      <a-descriptions-item label="应用密钥">{{ newTableItemInfo.appSecret }}</a-descriptions-item>
    </a-descriptions>
  </dialog-box>
  <dialog-box v-model:visible="editTableItemBoxVisible" title="编辑应用信息" :width="400">
    <div style="width: 200px; margin: 0 auto;">
      <a-form-item label="有效性">
        <a-switch v-model:checked="tableDataItemDisabled" />
      </a-form-item>
      <a-form-item>
        <a-button html-type="submit" type="primary" block @click="submitEdit">提交修改</a-button>
      </a-form-item>
    </div>
  </dialog-box>
</template>
